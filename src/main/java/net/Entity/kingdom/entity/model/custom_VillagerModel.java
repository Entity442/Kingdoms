...
public class custom_VillagerModel<T extends BasicVillager> extends EntityModel<T> {
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition villagerRightForearm = villagerRightArm.addOrReplaceChild("villagerRightForearm", CubeListBuilder.create().texOffs(48, 10).addBox(-2.0F, 0.0F, -2.0F, 4.0F, 6.0F, 4.0F, new CubeDeformation(-0.01F)), PartPose.offset(-1.0F, 4.0F, 0.0F));
		...
		PartDefinition villagerLeftforearm = villagerLeftArm.addOrReplaceChild("villagerLeftforearm", CubeListBuilder.create().texOffs(48, 10).addBox(-2.0F, 0.0F, -2.0F, 4.0F, 6.0F, 4.0F, new CubeDeformation(-0.01F)), PartPose.offset(1.0F, 4.0F, 0.0F));
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.villagerLeftHip.xRot = Mth.cos(limbSwing * 0.5F) * -0.5F * limbSwingAmount;
		this.villagerRightHip.xRot = Mth.cos(limbSwing * 0.5F) * 0.5F * limbSwingAmount;
		this.villagerRightArm.xRot = Mth.cos(limbSwing * 0.3331F + (float) Math.PI) * limbSwingAmount;
		this.villagerLeftArm.xRot = Mth.cos(limbSwing * 0.3331F) * limbSwingAmount;
		this.VillagerHead.yRot = netHeadYaw / (180F / (float) Math.PI);
		this.VillagerHead.xRot = headPitch / (180F / (float) Math.PI);
	}
